...
test = os.path.join(profile, 'cach')
...
def index():
        vauth = addon.getSetting('vinstall')
        vmail = addon.getSetting('vmail')
        valmac = pkcs5.get_mac()
        if vauth == 'false' :
                DIALOG.ok(ADDON_ID, "[COLOR %s]Please send above key to this e-mail: %s[/COLOR]" % ('cyan',vmail), "[COLOR %s]%s[/COLOR]" % ('cyan', valmac),
                  "[COLOR %s]You will receive on your mail a  password code[/COLOR]" % ('cyan'))

                mess1 = 'Input password code that you get on your e-mail '
                vmess1 = getKeyboard( default = "", heading = mess1, hidden=False)
                if pkcs5.get_hotp(valmac,int(1)) == int(vmess1) :
                        addon.setSetting('vinstall', 'true')
                        f = open(test,'w')
                        f.write(vmess1)
                        f.close()
                        ...
                        DIALOG.notification(ADDON_ID, "[COLOR %s]Wrong password[/COLOR]" % ('cyan'))
                        sys.exit()
    
        else :
                if not os.path.isfile(test)  :
                    addon.setSetting('vinstall', 'false')
                    sys.exit()
                ...
                vread = f.read()
                f.close()
                if pkcs5.get_hotp(valmac,int(1)) != int(vread) :
                    addon.setSetting('vinstall', 'false')
                    sys.exit()
